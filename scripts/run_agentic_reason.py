...
def main():
    ...
    # define tools
    print(f'-------------{args.remote_model}-------------')
    code_tool = code_agent(model_name=args.remote_model)
    search_tool = search_agent(llm, tokenizer, google_api_key, google_cse_id, google_endpoint, top_k = args.top_k, use_jina = use_jina, jina_api_key = jina_api_key, max_doc_len = max_doc_len, max_tokens = max_tokens, coherent = True, MAX_SEARCH_LIMIT = MAX_SEARCH_LIMIT, MAX_TURN = MAX_TURN)
