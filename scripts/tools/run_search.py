from tools.google_search import google_web_search, extract_relevant_info, extract_snippet_with_context, fetch_page_content
...
class search_agent:
    def __init__(self, llm, tokenizer, google_api_key, google_cse_id, google_endpoint, top_k = 10, output_records = [], all_urls_to_fetch = set(), url_snippets = {}, mind_map = None, search_cache = {}, url_cache = {}, use_jina = True, jina_api_key = None, max_doc_len = 1000, max_tokens = 8192, coherent = True, MAX_SEARCH_LIMIT = 10, MAX_TURN = 10):
        """
    ...
    def google_search(self, google_api_key, google_cse_id, google_endpoint, search_query, top_k = 10):
        """
        Execute google search and update caches
        """
        try:
            results = google_web_search(search_query, google_api_key, google_cse_id, google_endpoint, market='en-US', language='en')
    ...
    def __call__(self, context, search_query):
        ...
        relevant_info, url, snippets = self.google_search(self.google_api_key, self.google_cse_id, self.google_endpoint, search_query, top_k = self.top_k)
