...
class RemoteAPILLM:
    def __init__(self, model_name: str):
        ...
        if 'gpt' in model_name:
            ...
            if not self.api_key:
                raise ValueError("DeepSeek API key required. Set via environment variable 'DEEPSEEK_API_KEY'.")
            # DeepSeek API uses OpenAI-compatible client; map to the deployed model name
            self.client = OpenAI(api_key=self.api_key, base_url="https://api.deepseek.com")
            # Common public model id for chat; adjust here if your account uses a different deployment
            self.model_name = 'deepseek-chat' # or 'deepseek-reasoner'
    ...
    def generate(self, prompts: List[str], sampling_params: Optional[SamplingParams] = None, **kwargs) -> List:
        ...
        for prompt in prompts:
            ...
            elif self.provider == 'anthropic':  # anthropic
